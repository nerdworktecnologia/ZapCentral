...
import { Zap, ExternalLink, Loader2, RefreshCw, CheckCircle2, XCircle, Clock } from "lucide-react";
...
export default function Automacao() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: () => {
      toast({ title: "Erro ao alterar workflow.", variant: "destructive" });
    },
  });
    ...
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Workflows do n8n em tempo real</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Card>
          <CardContent className="p-5">
            <div className="text-sm text-muted-foreground">Total de workflows</div>
            <div className="text-2xl font-semibold mt-1">{workflows.length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
        ...
        <Card>
          <CardContent className="p-5">
            ...
            <div className="text-2xl font-semibold mt-1">{workflows.length - ativos}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
            ...
            workflows.map((w) => (
              <Card
                ...
                className={`cursor-pointer transition-all rounded-xl ${
                  selectedId === w.id ? "border-primary ring-1 ring-primary" : ""
                } ${!w.active ? "opacity-60" : ""}`}
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground">
                          Atualizado {formatDate(w.updatedAt)}
                        </div>
                      </div>
                    </div>
                    <Switch
        ...
        <div className="space-y-3">
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">
            {selectedId
              ? `Execuções — ${workflows.find((w) => w.id === selectedId)?.name ?? selectedId}`
              : "Execuções recentes"}
          </h2>
          {loadingExec ? (
            ...
            <Card className="rounded-xl">
              <CardContent className="p-5 text-sm text-muted-foreground">
                {selectedId ? "Nenhuma execução encontrada para este workflow." : "Nenhuma execução recente."}
              </CardContent>
            </Card>
          ) : (
            executions.map((ex) => (
