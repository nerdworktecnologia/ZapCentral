...
import {
  Send, Search, MessageSquare, CheckCheck, ExternalLink,
          ...
          <div className="h-14 px-4 flex items-center justify-between gap-3 border-b bg-card/50 shrink-0">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground">#{selected.id} · {statusLabel[selected.status]}</div>
              </div>
            </div>
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <a
                href={`${chatwoot.CW_URL}/app/accounts/${chatwoot.CW_ACCOUNT}/conversations/${selected.id}`}
                ...
                return (
                  <div key={msg.id} className={`flex ${isIncoming ? "justify-start" : "justify-end"}`}>
                    <div className={`max-w-[75%] rounded-2xl px-4 py-2.5 text-sm ${
                      isIncoming ? "bg-card text-foreground rounded-bl-md border" : "bg-primary text-primary-foreground rounded-br-md"
                    }`}>
                      {!isIncoming && msg.sender?.name && (
                        <div className="text-[10px] text-primary-foreground/70 font-medium mb-1">{msg.sender.name}</div>
                      )}
